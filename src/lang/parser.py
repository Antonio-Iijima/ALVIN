...
##### Complete ALVIN <-> Python converters #####



def toPython(expr: str) -> list:
    """Convert ALVIN syntax to Python list."""
...
def toALVIN(s: any) -> str | None:
    """Convert Python list to fully-parenthesized ALVIN string."""

    return (
        None if s is None
        else ("#t" if s else "#f") if isinstance(s, bool)
        else f"'{toALVIN(s[1])}" if KEY.isquote(s) 
        else f"({' '.join(toALVIN(elem) for elem in s if elem != None)})" if isinstance(s, list) 
        else str(s)
    )
